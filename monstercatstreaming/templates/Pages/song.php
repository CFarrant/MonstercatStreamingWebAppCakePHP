<?php
  ...
  $songdata = $songs->find('all')->where(['track_id' => $_GET['id']])->toArray();
  $albumdata = $albums->find('all')->where(['album_id' => $songdata[0]['album_id']])->toArray();
  $album_cover_url = $albumdata[0]['cover_url'];
  $song_title = $songdata[0]['title'];
  $song_artist = $songdata[0]['artist_name'];
  $song_genre_primary = $songdata[0]['genre_primary'];
  $song_genre_secondary = $songdata[0]['genre_secondary'];
    ...
    <div class="song">
      <span>
        <div style="margin-bottom: 5px;">
          <?php echo "<img style='width: 550px; height: 550px;' src='$album_cover_url'>" ?>
        </div>
        <br/>
        <div>
          <?php echo "<h1>$song_artist ~ $song_title</h1>" ?>
          <br />
          <?php echo "<h3>$song_genre_primary, $song_genre_secondary</h3>" ?>
        </div>
      </span>
    </div>
  </div>
